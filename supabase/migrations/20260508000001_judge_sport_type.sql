-- Add sport_type to judges; existing judges are acro by default.
ALTER TABLE public.judges
  ADD COLUMN IF NOT EXISTS sport_type text NOT NULL DEFAULT 'acro';

-- Update trigger to propagate sport_type from invite metadata.
...
begin
  ...
  if v_role in ('admin', 'super_admin') then
    ...
    values (
      ...
      new.raw_user_meta_data->>'phone',
      coalesce(new.raw_user_meta_data->>'sport_type', 'acro')
    )
    on conflict (id) do nothing;
