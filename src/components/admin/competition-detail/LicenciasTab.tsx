...
import type { Team, Club, CompetitionEntry, Gymnast, Coach } from '@/components/admin/types'
...
export default function LicenciasTab({ lang, globalTeams, clubs, entries, competitionGymnasts, competitionCoaches, globalCoaches }: Props) {
  const t = T[lang]
...
function ClubBlock({
  lang, club, clubCoaches, clubEntries, teamMap, gymnastMap, onBulkDownload,
}: {
  lang: Lang
  club: Club | undefined
  clubCoaches: Coach[]
  ...
  const activeEntries  = clubEntries.filter(e => !e.dropped_out)
  const droppedEntries = clubEntries.filter(e => e.dropped_out)
