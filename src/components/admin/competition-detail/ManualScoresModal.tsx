    ...
    try {
      ...
      const res = await fetch('/api/admin/scores', {
        ...
        body: JSON.stringify({
          ...
          teamId: team.id,
          publishResult,
          resultStatus: publishResult ? resultStatus : undefined,
          scores: rows.map((r) => ({
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          ...
          {saved && !error && (
            <p className="text-xs text-green-700 bg-green-50 border border-green-100 rounded-lg px-3 py-2">
              {publishResult ? t.savedPublished : t.saved}
            </p>
          )}
            ...
            <div className="rounded-xl border border-slate-200 p-3 bg-white space-y-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) => { setPublishResult(e.target.checked); setSaved(false) }}
                  className="rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm text-slate-700">{t.publishResult}</span>
              </label>
              {publishResult && (
              ...
              <p className="text-[11px] text-slate-400 pl-6">{t.publishHint}</p>
            </div>
          )}
        </div>
