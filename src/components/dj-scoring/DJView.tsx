...
function calcTotals(
  ...
  return { difficulty: parseFloat(difficulty.toFixed(2)), penalty: parseFloat(penalty.toFixed(1)) }
}
...
function PerformanceHeader({ perf, lang, difficulty, penalty }: {
  perf: Performance; lang: Lang; difficulty: number; penalty: number
}) {
  const t = T[lang]
...
function Toggle({ active, onToggle, labelOff, labelOn, color = 'red' }: {
  active: boolean; onToggle: () => void; labelOff: string; labelOn: string; color?: 'red' | 'amber' | 'blue'
}) {
  const colorMap = {
...
function FlagRow({ flag, elementId, attemptNumber, flags, lang, config, onChange, onOpenRetry }: {
  flag: ElementFlag; elementId: string; attemptNumber: number; flags: ElementFlags
  lang: Lang; config: ElementConfig
  ...
  return (
    <div className={attemptNumber > 1 ? 'pl-3 border-l-2 border-slate-200 mt-2' : ''}>
      ...
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        <button onClick={() => patch({ isDone: flag.isDone === true ? null : true })}
          className={['px-2.5 py-1 rounded-lg text-xs font-medium border transition-all',
            flag.isDone === true ? 'bg-emerald-500 text-white border-emerald-500' : 'bg-white text-slate-400 border-slate-200 hover:border-emerald-300 hover:text-emerald-600'].join(' ')}>
          ✓ {t.done}
        </button>
        <button onClick={() => patch({ isDone: flag.isDone === false ? null : false })}
          className={['px-2.5 py-1 rounded-lg text-xs font-medium border transition-all',
            flag.isDone === false ? 'bg-amber-400 text-white border-amber-400' : 'bg-white text-slate-400 border-slate-200 hover:border-amber-300 hover:text-amber-600'].join(' ')}>
          ✗ {t.notDone}
        </button>
        {config.showTf && (
          <>
      ...
      <div className="flex flex-wrap gap-1.5 mb-2">
        <Toggle active={flag.srNotDone} onToggle={() => patch({ srNotDone: !flag.srNotDone })} labelOff={t.srNotDone} labelOn={t.srNotDone} color="red" />
        {config.showNoSupport && (
          <Toggle active={flag.landingWithoutSupport} onToggle={() => patch({ landingWithoutSupport: !flag.landingWithoutSupport })} labelOff={t.noSupport} labelOn={t.noSupport} color="red" />
        )}
        <Toggle active={flag.forbiddenElement} onToggle={() => patch({ forbiddenElement: !flag.forbiddenElement })} labelOff={t.forbidden} labelOn={t.forbidden} color="red" />
      ...
      {showNote && (
        <input type="text" value={flag.note} onChange={(e) => patch({ note: e.target.value })}
          placeholder={t.note}
      ...
      {canAddRetry && (
        <button onClick={() => onOpenRetry(elementId, maxAttempt + 1)}
...
function ElementRow({ element, flags, lang, onChange, onOpenRetry, isExtra, onLabelChange, onTypeChange }: {
  element: TsElement; flags: ElementFlags; lang: Lang
  onChange: (elementId: string, attemptNumber: number, patch: Partial<ElementFlag>) => void
  onOpenRetry: (elementId: string, nextAttemptNumber: number) => void
  isExtra?: boolean
  ...
  return (
    <div className={['border rounded-xl p-3 bg-white transition-colors',
      isDone === false ? 'border-amber-200 bg-amber-50/30' : 'border-slate-100'].join(' ')}>
        ...
        <div className="flex items-center gap-1.5 mb-2 flex-wrap">
          {(['balance', 'mount', 'dynamic', 'individual', 'motion'] as ElementType[]).map((type) => (
            <button key={type} onClick={() => onTypeChange(element.id, type)}
              className={['px-2.5 py-1 rounded-lg text-xs font-medium border transition-all capitalize',
                element.elementType === type ? 'bg-slate-700 text-white border-slate-700' : 'bg-white text-slate-400 border-slate-200 hover:border-slate-400'].join(' ')}>
              {type}
            </button>
          ))}
          {element.elementType === 'individual' && (
            <button onClick={() => onTypeChange(element.id, 'individual', !element.isStatic)}
              className={['px-2.5 py-1 rounded-lg text-xs font-medium border transition-all',
                element.isStatic ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-slate-400 border-slate-200 hover:border-blue-300'].join(' ')}>
              {t.isStatic}
            </button>
          )}
        </div>
      ...
      {Array.from({ length: maxAttempt }, (_, i) => i + 1).map((attempt) => {
        ...
        return (
          <FlagRow key={attempt} flag={flag} elementId={element.id} attemptNumber={attempt}
    ...
    <div className="px-4 pb-8">
          ...
          return (
            <button key={field} onClick={() => setFocused(field)}
              className={['rounded-2xl py-4 text-center transition-all border-2',
                isFocused ? 'border-slate-800 bg-white shadow-sm' : 'border-transparent bg-slate-100'].join(' ')}>
      ...
      <button disabled={!canSubmit} onClick={() => canSubmit && onSubmit(dScore!, pScore!)}
        className={['w-full py-4 rounded-2xl font-bold text-lg transition-all',
          canSubmit ? 'bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-white' : 'bg-slate-100 text-slate-300 cursor-not-allowed'].join(' ')}>
...
function PdfPanel({ tsUrl, lang }: { tsUrl: string | null | undefined; lang: Lang }) {
  ...
  return tsUrl ? (
    ...
    <div className="w-full h-full border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center gap-3 bg-white text-slate-400 min-h-0">
...
export default function DJView({ currentPerf, lang, elements, mode = 'elements', onSubmit, waitingForOtherScores, judgeScores, panelJudges, result }: DJViewProps) {
  ...
  function handleChange(elementId: string, attemptNumber: number, patch: Partial<ElementFlag>) {
    ...
    const element = orderedAll.find((el) => el.id === elementId)
  ...
  function handleAddElement() {
    ...
    setOrderedAll((prev) => [...prev, { id, position: 0, label: '', difficultyValue: 0 }])
    setFlags((prev) => ({ ...prev, [`${id}:1`]: { ...DEFAULT_FLAG } }))
  }

  function handleLabelChange(id: string, label: string) {
    setOrderedAll((prev) => prev.map((el) => el.id === id ? { ...el, label } : el))
  }

  function handleTypeChange(id: string, type: ElementType, isStatic?: boolean) {
    setOrderedAll((prev) => prev.map((el) => el.id === id ? { ...el, elementType: type, isStatic: isStatic ?? el.isStatic } : el))
  }
  ...
  // ── elements mode: element list always visible, PDF shows alongside on wide screens ──
  return (
    <div className="h-full px-4 pb-4">
      <div className="flex gap-4 h-full">
        ...
        <div className="flex-1 md:flex-none md:w-[420px] flex flex-col gap-3 min-h-0">
          <PerformanceHeader perf={currentPerf} lang={lang} difficulty={difficulty} penalty={penalty} />

          {/* incorrect TS toggle */}
          <button onClick={() => setIncorrectTs((v) => !v)}
            className={['flex items-start gap-3 px-3 py-2.5 rounded-xl border text-left transition-all',
              incorrectTs ? 'border-red-300 bg-red-50' : 'border-slate-200 bg-white hover:border-slate-300'].join(' ')}>
            <div className={['w-4 h-4 rounded border mt-0.5 shrink-0 flex items-center justify-center',
              incorrectTs ? 'bg-red-500 border-red-500' : 'border-slate-300'].join(' ')}>
          ...
          {/* element list with drag-drop for unlisted elements */}
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-0">
            {orderedAll.length === 0 ? (
              ...
              orderedAll.map((el, idx) => {
                ...
                const isDropTarget = dragId && !isDragging && dropIdx === idx
                return (
                  <div
                    key={el.id}
                    draggable={isExtra}
                    ...
                    className={[
                      ...
                      isDropTarget ? 'ring-2 ring-blue-400 ring-offset-1 rounded-xl' : '',
                    ].join(' ')}
                  >
                    {isExtra && <div className="text-xs text-slate-300 text-right pr-1 mb-0.5 select-none">⠿ drag to reorder</div>}
                    <ElementRow element={el} flags={flags} lang={lang}
                      onChange={handleChange} onOpenRetry={handleOpenRetry}
                      isExtra={isExtra}
                      onLabelChange={isExtra ? handleLabelChange : undefined}
                      onTypeChange={isExtra ? handleTypeChange : undefined} />
                  </div>
                )
              })
            )}
          ...
          <button onClick={handleSubmitElements}
