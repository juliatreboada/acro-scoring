...
function DualKeypad({ lang, perf, onSubmit }: {
  ...
  return (
    <div className="px-4 pb-8">
      <ScoringPerformanceHeader perf={perf} lang={lang} mb="mb-3">
        <div className="flex items-center gap-3 text-sm font-bold tabular-nums">
          <span className="text-emerald-400">D {(dScore ?? 0).toFixed(2)}</span>
          {(pScore ?? 0) > 0 && <span className="text-red-400">−{(pScore ?? 0).toFixed(1)}</span>}
        </div>
