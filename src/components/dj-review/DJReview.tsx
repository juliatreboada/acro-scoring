...
export default function DJReview({ initialSheets, myJudgeId, lang, practiceMode = false, judgeLobbyHref }: DJReviewProps) {
  ...
  return (
    <>
      ...
      {modalSheet && (() => {
        const modalIdx = sheets.findIndex(s => s.id === modalSheetId)
        const prevSheet = modalIdx > 0 ? sheets[modalIdx - 1] : null
        const nextSheet = modalIdx < sheets.length - 1 ? sheets[modalIdx + 1] : null
        return (
        <div className="fixed inset-0 z-50 bg-white flex flex-col">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-200 shrink-0">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <span className="text-xs text-slate-400 tabular-nums mr-1">{modalIdx + 1}/{sheets.length}</span>
              <button
                onClick={() => prevSheet && setModalSheetId(prevSheet.id)}
                disabled={!prevSheet}
              ...
              <button
                onClick={() => nextSheet && setModalSheetId(nextSheet.id)}
                disabled={!nextSheet}
                className="p-1.5 rounded-lg text-slate-500 hover:text-slate-800 hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
