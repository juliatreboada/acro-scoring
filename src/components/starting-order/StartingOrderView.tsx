      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-6">
          ...
          (() => {
            ...
            const onePanelItems: OnePanelItem[] = []
            const timelineSlots: PerfSlot[] = []
            const droppedIds = new Set(entries.filter(e => e.dropped_out).map(e => e.team_id))

            if (currentSection?.timeline_order && currentSection.timeline_order.length > 0) {
              const seenSessions = new Set<string>()
              for (const entry of currentSection.timeline_order) {
                if ('type' in entry && entry.type === 'break') {
                  onePanelItems.push({ kind: 'break', duration_minutes: entry.duration_minutes, label: entry.label })
                  timelineSlots.push({ kind: 'break', duration_minutes: entry.duration_minutes, label: entry.label })
                  continue
                }
                const te = entry as { session_id: string; team_id: string }
                const sess = panelSessions.find(s => s.id === te.session_id)
                if (!sess) continue
                if (!seenSessions.has(te.session_id)) {
                  seenSessions.add(te.session_id)
                  onePanelItems.push({ kind: 'session', session: sess })
                }
                const orders = sessionOrders.filter(o => o.session_id === te.session_id).sort((a, b) => a.position - b.position)
                const alreadyAdded = timelineSlots.some(s => s.kind === 'team' && s.sessionId === te.session_id && s.teamId === te.team_id)
                if (!alreadyAdded) {
                  const o = orders.find(o => o.team_id === te.team_id)
                  if (o) {
                    timelineSlots.push({ kind: 'team', panelNumber: 1, sessionId: sess.id, sessionName: sess.name, teamId: o.team_id, isDropout: droppedIds.has(o.team_id) })
                  }
                }
              }
              for (const sess of panelSessions) {
            ...
            const hasBreaks = onePanelItems.some(i => i.kind === 'break')
            const timesMap = currentSection
              ? (hasBreaks
                  ? calcTimeFromMerged(currentSection, timelineSlots, sessions, ageGroupRules)
                  : calcPanelTimes(currentSection, panelSessions, sessionOrders, ageGroupRules))
              : new Map<string, SlotTimes>()

            return (
              <div className="space-y-4">
                {onePanelItems.map((item, i) => {
                  if (item.kind === 'break') {
                    return (
                      <div key={`break-${i}`} className="flex items-center gap-3 px-4 py-2.5 bg-amber-50 rounded-2xl border border-amber-100">
                        <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 bg-amber-100 text-amber-600">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-semibold text-amber-700">{item.label ?? t.breakLabel}</p>
                          <p className="text-xs text-amber-500">{item.duration_minutes} min</p>
                        </div>
                      </div>
                    )
                  }
                  return (
                    <SessionOrderCard
                      ...
                      isLocked={lockedSessions.includes(item.session.id)}
                      entries={entries}
