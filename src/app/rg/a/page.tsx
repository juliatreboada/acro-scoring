...
export default function Page() {
  ...
  const myRole = assignedRoles.find(r => r.role === 'A')
  ...
  function handleSubmit(value: number) {
    if (!myRole) return
    const s: JudgeScore = {
      panelJudgeId: myRole.id, ajScore: value,
      ejScore: null, djDifficulty: null, djPenalty: null, cjpPenalty: null,
    }
    ...
    <JudgeScoringShell loading={loading} sessionId={sessionId} lang={lang} onLangChange={setLang} submitError={submitError} onClearError={clearSubmitError}>
      <div className="px-0 md:px-4 pt-4 md:flex-1 md:min-h-0 overflow-y-auto">
        <RGJudgeView
          ...
          mySubmittedScore={myScore?.ajScore ?? null}
