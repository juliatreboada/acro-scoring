...
export default function Page() {
  ...
  async function handleAssignSessionMergeGroup(sessionId: string, mergeGroupId: string | null) {
    ...
    if (mergeGroupId) {
      ...
      if (conflict) {
        setActionError(
          lang === 'en'
            ? 'This merge group already has sessions with a different age group or routine type.'
            : 'Este grupo ya tiene sesiones con otro grupo de edad o tipo de rutina.',
        )
        return
      }
    }
    ...
    if (error) {
      console.error(error)
      setActionError(lang === 'en' ? 'Could not update session.' : 'No se pudo actualizar la sesión.')
      return
    }
    setSessions((prev) =>
  ...
  async function handleCreateRankingMergeGroup(labelEs: string, labelEn: string): Promise<string | null> {
    ...
    if (error || !data) {
      console.error(error)
      setActionError(lang === 'en' ? 'Could not create merge group.' : 'No se pudo crear el grupo.')
      return null
    }
