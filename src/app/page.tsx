...
type Competition = Pick<
  ...
  'id' | 'slug' | 'name' | 'status' | 'location' | 'start_date' | 'end_date' | 'age_groups' | 'poster_url'
>
      ...
      <div className="w-full sm:w-[65%] sm:min-w-0 min-w-0 flex flex-col justify-between gap-2.5 px-3 py-3 sm:py-4 sm:px-4">
        ...
        {(isActive || isFinished) && (
          <div className="flex flex-col gap-1.5 pt-2 border-t border-slate-100 sm:items-stretch">
            ...
            <button
              type="button"
              onClick={() => router.push(`/results/${comp.slug}`)}
              className={[
...
const ROLE_BUTTON_CLS: Record<string, string> = {
  super_admin: 'bg-violet-500 hover:bg-violet-600',
  admin:       'bg-blue-500 hover:bg-blue-600',
  judge:       'bg-amber-500 hover:bg-amber-600',
  club:        'bg-emerald-500 hover:bg-emerald-600',
}
...
export default function HomePage() {
  ...
  const t = useT('HomePage', lang)
  const { activeProfile, allProfiles, switchProfile, profileLoading } = useProfile()
  ...
  useEffect(() => {
    async function load() {
      const [{ data }, { data: rulesData }] = await Promise.all([
        supabase
          .from('competitions')
          .select('id, slug, name, status, location, start_date, end_date, age_groups, poster_url')
          .neq('status', 'draft')
            ...
            {!profileLoading && activeProfile ? (
              <div className="relative" ref={profileRef}>
                <button
                  ...
                  className={['flex items-center gap-2.5 pl-1.5 pr-3 py-1.5 rounded-full transition-all', ROLE_BUTTON_CLS[activeProfile.role] ?? 'bg-slate-500 hover:bg-slate-600'].join(' ')}
                >
                  <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-white text-xs font-bold ring-2 ring-white/30">
                    {activeProfile.avatar_url
                      ? <img src={activeProfile.avatar_url} alt={activeProfile.name} className="w-full h-full object-cover" />
                      : <div className={['w-full h-full flex items-center justify-center text-white text-xs font-bold', ROLE_BADGE_BG[activeProfile.role] ?? 'bg-slate-500'].join(' ')}>{avatarInitials(activeProfile.name)}</div>}
                  </div>
                  <span className="text-xs font-semibold text-white">{ROLE_LABEL[activeProfile.role]}</span>
                ...
                {profileOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-xl py-1.5 z-50">
                    {allProfiles.map(p => (
                      <button
                        key={p.id}
                        onClick={() => { setProfileOpen(false); switchProfile(p.id); router.push(ROLE_HOME[p.role]) }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 hover:bg-slate-50 transition-colors text-left"
                      >
                        <div className={['w-7 h-7 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-white text-[10px] font-bold', ROLE_BADGE_BG[p.role] ?? 'bg-slate-500'].join(' ')}>
                          {p.avatar_url
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500 font-medium">{ROLE_LABEL[p.role]}</p>
                        </div>
