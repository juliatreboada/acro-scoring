...
export async function POST(req: NextRequest) {
  ...
  if (role === 'judge') {
    const { error } = await supabaseAdmin.from('judges').insert({
      id: newProfileId,
    ...
    if (profErr) {
      await supabaseAdmin.from('judges').delete().eq('id', newProfileId)
    ...
    if (profErr) {
      await supabaseAdmin.from('admins').delete().eq('id', newProfileId)
      return NextResponse.json({ error: profErr.message }, { status: 500 })
    }
...
export async function PATCH(req: NextRequest) {
  ...
  if (profile.role === 'judge') {
    ...
    if (Object.keys(u).length) await supabaseAdmin.from('judges').update(u as any).eq('id', profileId)
  } else if (profile.role === 'admin') {
    const u: Record<string, unknown> = {}
    if (full_name !== undefined) u.full_name = full_name
    if (phone     !== undefined) u.phone     = phone || null
    if (Object.keys(u).length) await supabaseAdmin.from('admins').update(u as any).eq('id', profileId)
    ...
    if (phone        !== undefined) u.phone        = phone || null
    if (Object.keys(u).length) await supabaseAdmin.from('clubs').update(u as any).eq('id', profile.club_id)
  }
...
export async function DELETE(req: NextRequest) {
  ...
  if (profile.role === 'judge') {
    await supabaseAdmin.from('judges').delete().eq('id', profileId)
  } else if (profile.role === 'admin') {
    await supabaseAdmin.from('admins').delete().eq('id', profileId)
  }
